...
export class CallService {
    ...
    public getCalls(refresh: boolean = false): Observable<Call[]> {
        ...
        return this.http.get(firebaseDbUrl).pipe(
            mergeMap((h: { [id: string]: any }) => {
                ...
                return forkJoin(
                    Object.entries<{ [id: string]: any }>(h).map(
                        ([id, callDb]) => {
                            if (!callDb.GeckoURL) {
    ...
    dispatchCallInfo(GeckoData: any): CallInfo {
        return {
            ...
            sentiment_votes_up_percentage:
                GeckoData.sentiment_votes_up_percentage,
    ...
    public addCallToDB(call?: CallDb): Observable<void> {
        ...
        this.http
            .post<AddResult>(
                `${this.firebaseURL}/${this.firebaseDB}.json`,
                callToAdd
            )
    ...
    updateCall(call: CallDTO): void {
        this.http
            .patch(
                `${this.firebaseURL}/${this.firebaseDB}/${call[0]}.json`,
                call[1]
            )
    ...
    private deleteCallFromList(id: string): void {
        const index: number = this.calls.findIndex(
